#!/usr/bin/env python3
# Mejora final V3.1 del motor editorial de Estrecho Ormuz.
...
FILES = {
    ...
    "decision_tests": ROOT / "test_decision_engine.py",
}
...
def patch_decision_tests(text: str) -> str:
    """Insert regression tests before unittest.main(), regardless of line endings."""
    if "ORMUZ_V3_FINAL_TESTS_IMPORTED" in text:
        compile(text, str(FILES["decision_tests"]), "exec")
        return text
    ...
    if main_guard is None:
        ...
        insertion_point = main_guard.start()

    prefix = text[:insertion_point].rstrip()
    suffix = text[insertion_point:].lstrip()
    updated = (
        prefix
        + "\n\n"
        + DECISION_TEST_IMPORT.strip()
        + "\n\n"
        + suffix
    )
    compile(updated, str(FILES["decision_tests"]), "exec")
    return updated
...
def run_tests() -> None:
    for file in (
        FILES["engine"],
        FILES["guard"],
        ROOT / "maintenance_common.py",
        FILES["tests"],
        FILES["decision_tests"],
    ):
...
def apply() -> None:
    ...
    try:
        ...
        atomic_write(
            FILES["decision_tests"],
            patch_decision_tests(original["decision_tests"]),
        )
        run_tests()
    except Exception:
        ...
        if "tests" not in original and FILES["tests"].exists():
            FILES["tests"].unlink()
        raise
    print("\nMEJORA FINAL V3.2 APLICADA CORRECTAMENTE")
    print(f"Respaldo creado fuera del repositorio: {backup}")
    for key in ("engine", "guard", "aliases", "tests", "decision_tests"):
